...
// > git restore <file_name> --> it is a command to discard the changes in the specified file and restore it to its previous state.
// > git restore . --> it is a command to discard all the changes in the current directory and restore all the files to their previous state.
...
// for this every commits have a unique id which is hash value so we can use that hash value to undo the changes of that commit.
// > git reset --soft <commit_hash> --> it is a command to undo the commits and keep the changes in the staging area.
// > git reset --hard <commit_hash> --> it is a command to undo the commits and discard all the changes in the working directory.


// showing the commit history of the repository
// > git log --> it is a command to show the commit history of the repository. it shows the commit hash, author, date, and commit message for each commit in the repository.
// > git log --oneline --> it is a command to show the commit history in a condensed format. it shows only the commit hash and commit message for each commit in a single line.
// > git log --graph --> it is a command to show the commit history in a graphical format. it shows the branches and merges in the commit history.


// git stash --> it is a command to temporarily save the changes that are not ready to be committed. it allows you to save your changes and revert to a clean working directory without committing the changes. you can later apply the stashed changes when you are ready to work on them again.
// > git stash save "message" --> it is a command to save the changes in the stash with a message that describes the changes.
// > git stash list --> it is a command to list all the stashed changes in the repository.
// > git stash apply --> it is a command to apply the most recent stashed changes to the working directory.
